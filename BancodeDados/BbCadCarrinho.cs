...
namespace Menu
{
	class DBCadCarrinho
	{
		...
		public DBCadCarrinho()
		{
			sqldb_message = "";
			sqldb_available = false;
		}

		public DBCadCarrinho(string sqldb_name)
		{
			try
			{
				sqldb_message = "";
		...
		public void CreateDatabase(string sqldb_name)
		{
			try
			{
				...
				sqldb_query = "CREATE TABLE IF NOT EXISTS carrinho (_codPedido INTEGER, _codEmpresa INTEGER, numMesa INTEGER, dataPedido VARCHAR,  " + 
					" valorTotal REAL, numSerial VARCHAR, numCelular VARCHAR, dataFechado TEXT, codigoMesa VARCHAR);";
				sqldb.ExecSQL(sqldb_query);
				sqldb_message = "Database: " + sqldb_name + " created";
				//}
				//else
		...
		//ADICIONA PEDIDO
		public void AddRecord(int iCodPedido, int iCodEmpresa, int iNumMesa, string sDataPedido, double dValorTotal, 
			string sNumSerial, string sNumCelular, string sCodigoMesa)
		{
			try
			{
				sqldb_query = "INSERT INTO carrinho (_codPedido, _codEmpresa, numMesa, dataPedido, valorTotal, numSerial, numCelular, codigoMesa) VALUES ('" + 
		...
		public void DeleteRecord()
		{
			try
			{
				sqldb_query = "DELETE FROM carrinho;";
				sqldb.ExecSQL(sqldb_query);
				sqldb_message = "Record deleted";
			}
			catch(SQLiteException ex) 
		...
		public Android.Database.ICursor GetRecordCursor()
		{
			...
			try
			{
				sqldb_query = "SELECT _codPedido, _codEmpresa numMesa, dataPedido, valorTotal, numSerial, numCelular FROM carrinho;";
		...
		public Android.Database.ICursor GetRecordCursor(int iCodPedido, int iCodEmpresa)
		{
			...
			try
			{
				sqldb_query = "SELECT _codPedido, _codEmpresa, numMesa, dataPedido, valorTotal, numSerial, numCelular " + 
					" FROM carrinho WHERE _codPedido ='" + iCodPedido + "' and _codEmpresa = '" + iCodEmpresa + "';";
				sqldb_cursor = sqldb.RawQuery(sqldb_query, null);
				if(!(sqldb_cursor != null))
				{
					sqldb_message = "Record not found";
